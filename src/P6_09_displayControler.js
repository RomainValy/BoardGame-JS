...
class DisplayManager {
  ...
  boardGenerator() {
    ...
    for (let x = 0; x < this.boardGame.lines; x++) {
      ...
      for (let y = 0; y < this.boardGame.columns; y++) {
        $("#board").append(
          '<span id="' +
            this.boardGame.cases[x][y].x +
            "," +
            this.boardGame.cases[x][y].y +
            '" class=' +
            this.boardGame.cases[x][y].cssClass +
            "></span>"
        );
      }
    }
  ...
  fichePerso(id, player) {
    ...
    $("#" + id).append(
      "<p> Votre arme " + player.name + ":<br>" + player.arme.name + "</p>"
    );
  ...
  switchTourButton() {
    ...
    switchTourInput.addEventListener("click", () => {
      ...
      if (
        (this.boardGame.cases[this.boardGame.currentPlayer.x + 1][
          this.boardGame.currentPlayer.y
        ] !== undefined &&
          this.boardGame.cases[this.boardGame.currentPlayer.x + 1][
            this.boardGame.currentPlayer.y
          ] ===
            this.boardGame.cases[this.boardGame.enemy.x][
              this.boardGame.enemy.y
            ]) ||
        (this.boardGame.cases[this.boardGame.currentPlayer.x - 1][
          this.boardGame.currentPlayer.y
        ] !== undefined &&
        ...
        (this.boardGame.cases[this.boardGame.currentPlayer.x][
          this.boardGame.currentPlayer.y + 1
        ] !== undefined &&
          this.boardGame.cases[this.boardGame.currentPlayer.x][
            this.boardGame.currentPlayer.y + 1
          ] ===
  ...
  // ouvre 2 fenetre pour que les jouer entre leur nom. si null, retourne le nom par defaut
  selectPseudoPlayers() {
  ...
  // method qui refresh le Dom avec les elements MAJ
  refreshBoardGame() {
    $("#board").remove();
