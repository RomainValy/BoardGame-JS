/** @format */
/**
 * @class gère toute la partie "combat"
 */
class Battle {
  /**
   *
   * @param {object} currentPlayer => joueur actuel
   * @param {object} enemy => joueur dont ce n'est pas le tour
   * @param {object} displayManager
   */
  ...
  // gere l'interface combat
  attack() {
    // les deux joueurs choississent l'option "attaque"
    if (
      this.currentPlayer.fightAction === "attack" &&
      this.enemy.fightAction === "attack"
    ) {
    ...
    // Jouer 1 defend et joueur 2 attaque
      ...
      this.currentPlayer.fightAction === "defend" &&
      this.enemy.fightAction === "attack"
    ) {
      this.currentPlayer.hp = this.currentPlayer.hp -=
        this.enemy.arme.damage * 0.5;
  ...
  // gestion de la logique tour par tour en fonction des PV
  fightScenario() {
    if (this.currentPlayer.hp > 0 && this.enemy.hp > 0) {
      ...
      this.currentPlayer.hp < this.enemy.hp
        ? alert(this.enemy.name + " VAINQUEUR!")
        : alert(this.currentPlayer.name + " VAINQUEUR!");
      document.location.reload(true);
    }
  }
  // modifie le DOM pour la phase de combat
  fightBoard() {
    ...
    $("#gameSpace").append(
      '<div id="fiche_player1" class="fichesPlayers" ></div>'
    );
    $("#gameSpace").append('<img src="./img/imgCombat.jpg" class="imgCombat">');
    $("#gameSpace").append(
      '<div id="fiche_player2" class="fichesPlayers"></div>'
    );
  ...
  // créé et gère les input attack et defense
